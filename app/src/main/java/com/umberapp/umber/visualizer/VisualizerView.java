package com.umberapp.umber.visualizer;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.Bitmap.Config;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Matrix;
import android.graphics.Paint;
import android.graphics.PorterDuff.Mode;
import android.graphics.PorterDuffXfermode;
import android.graphics.Rect;
import android.media.MediaPlayer;
import android.media.MediaPlayer.OnCompletionListener;
import android.media.audiofx.Visualizer;
import android.media.audiofx.Visualizer.OnDataCaptureListener;
import android.util.AttributeSet;
import android.view.View;

import com.umberapp.umber.visualizer.renderer.Renderer;
import com.wdullaer.materialdatetimepicker.Utils;

import java.util.HashSet;
import java.util.Set;

public class VisualizerView extends View {
    public static final String TAG = "VisualizerView";
    private AudioData mAudioData;
    private byte[] mBytes;
    Canvas mCanvas;
    Bitmap mCanvasBitmap;
    private byte[] mFFTBytes;
    private Paint mFadePaint;
    private FFTData mFftData;
    boolean mFlash;
    private Paint mFlashPaint;
    private Matrix mMatrix;
    private Rect mRect;
    private Set<Renderer> mRenderers;
    private Visualizer mVisualizer;

    /* renamed from: com.umberapp.umber.visualizer.VisualizerView.1 */
    class C14731 implements OnDataCaptureListener {
        C14731() {
        }

        public void onWaveFormDataCapture(Visualizer visualizer, byte[] bytes, int samplingRate) {
            VisualizerView.this.updateVisualizer(bytes);
        }

        public void onFftDataCapture(Visualizer visualizer, byte[] bytes, int samplingRate) {
            VisualizerView.this.updateVisualizerFFT(bytes);
        }
    }

    /* renamed from: com.umberapp.umber.visualizer.VisualizerView.2 */
    class C14742 implements OnCompletionListener {
        C14742() {
        }

        public void onCompletion(MediaPlayer mediaPlayer) {
            VisualizerView.this.mVisualizer.setEnabled(false);
        }
    }

    public VisualizerView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs);
        this.mAudioData = null;
        this.mFftData = null;
        this.mRect = new Rect();
        this.mMatrix = new Matrix();
        this.mFlashPaint = new Paint();
        this.mFadePaint = new Paint();
        this.mFlash = false;
        init();
    }

    public VisualizerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VisualizerView(Context context) {
        this(context, null, 0);
    }

    private void init() {
        this.mBytes = null;
        this.mFFTBytes = null;
        this.mFlashPaint.setColor(Color.argb(122, Utils.SELECTED_ALPHA_THEME_DARK, Utils.SELECTED_ALPHA_THEME_DARK, Utils.SELECTED_ALPHA_THEME_DARK));
        this.mFadePaint.setColor(Color.argb(238, Utils.SELECTED_ALPHA_THEME_DARK, Utils.SELECTED_ALPHA_THEME_DARK, Utils.SELECTED_ALPHA_THEME_DARK));
        this.mFadePaint.setXfermode(new PorterDuffXfermode(Mode.MULTIPLY));
        this.mRenderers = new HashSet();
    }

    public void link(MediaPlayer player) {
        if (player == null) {
            throw new NullPointerException("Cannot link to null MediaPlayer");
        }
        this.mVisualizer = new Visualizer(player.getAudioSessionId());
        this.mVisualizer.setCaptureSize(Visualizer.getCaptureSizeRange()[1]);
        this.mVisualizer.setDataCaptureListener(new C14731(), Visualizer.getMaxCaptureRate() / 2, true, true);
        this.mVisualizer.setEnabled(true);
        player.setOnCompletionListener(new C14742());
    }

    public void addRenderer(Renderer renderer) {
        if (renderer != null) {
            this.mRenderers.add(renderer);
        }
    }

    public void clearRenderers() {
        this.mRenderers.clear();
    }

    public void release() {
        if (this.mVisualizer != null) {
            this.mVisualizer.release();
        }
    }

    public void updateVisualizer(byte[] bytes) {
        this.mBytes = bytes;
        invalidate();
    }

    public void updateVisualizerFFT(byte[] bytes) {
        this.mFFTBytes = bytes;
        invalidate();
    }

    public void flash() {
        this.mFlash = true;
        invalidate();
    }

    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        this.mRect.set(0, 0, getWidth(), getHeight());
        if (this.mCanvasBitmap == null) {
            this.mCanvasBitmap = Bitmap.createBitmap(canvas.getWidth(), canvas.getHeight(), Config.ARGB_8888);
        }
        if (this.mCanvas == null) {
            this.mCanvas = new Canvas(this.mCanvasBitmap);
        }
        this.mCanvas.drawColor(0);
        if (this.mBytes != null) {
            if (this.mAudioData == null) {
                this.mAudioData = new AudioData();
            }
            this.mAudioData.setBytes(this.mBytes);
            for (Renderer r : this.mRenderers) {
                r.render(this.mCanvas, this.mAudioData, this.mRect);
            }
        }
        if (this.mFFTBytes != null) {
            if (this.mFftData == null) {
                this.mFftData = new FFTData();
            }
            this.mFftData.setBytes(this.mFFTBytes);
            for (Renderer r2 : this.mRenderers) {
                r2.render(this.mCanvas, this.mFftData, this.mRect);
            }
        }
        this.mCanvas.drawPaint(this.mFadePaint);
        if (this.mFlash) {
            this.mFlash = false;
            this.mCanvas.drawPaint(this.mFlashPaint);
        }
        this.mMatrix.reset();
        canvas.drawBitmap(this.mCanvasBitmap, this.mMatrix, null);
    }
}
